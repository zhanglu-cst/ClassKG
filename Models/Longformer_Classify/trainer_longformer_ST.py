...
class Trainer_Longformer(Trainer_Base):
    ...
    def get_loader_from_origin_sentence_label(self, sentences, labels):
    ...
    def train_model(self, sentences, labels, ITR, finetune_from_pretrain = True):
        ...
        while itr_self_training < 10:
            dataloader_train = self.get_loader_from_origin_sentence_label(sentences, labels)
    ...
    def __do_train__(self, dataloader, ITR, itr_self_training):
            ...
            for iteration, batch in enumerate(dataloader):
                ...
                if total_itr % 10 == 0:
                    ...
                    self.logger.plot_record(value = meters.loss.median,
                                            win_name = 'classifier loss,itr:{} ST:{}'.format(ITR,
                                                                                             itr_self_training),
                                            X_value = total_itr)
                ...
                if (total_itr % self.cfg.classifier.eval_interval == 0):
                    ...
                    self.logger.plot_record(f1_micro,
                                            win_name = 'cls on all micro itr_{}_{}'.format(ITR, itr_self_training),
                                            X_value = total_itr)
                    self.logger.plot_record(f1_macro,
                                            win_name = 'cls on all macro itr_{}_{}'.format(ITR, itr_self_training),
                                            X_value = total_itr)
                    if (f1_micro > global_best):
                        global_best = f1_micro
                        self.checkpointer.save_to_checkpoint_file_with_name(self.model,
        ...
        self.logger.plot_record(f1_micro, win_name = 'cls on all micro itr_{}_{}'.format(ITR, itr_self_training),
                                X_value = total_itr)
        self.logger.plot_record(f1_macro, win_name = 'cls on all macro itr_{}_{}'.format(ITR, itr_self_training),
                                X_value = total_itr)
