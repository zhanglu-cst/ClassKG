...
cfg_file = 'agnews.yaml'
visdom_env_name = 'agnews'
...
def main(rank):
    ...
    for cur_itr in range(TOTAL_ITR):
        ...
        voted_sentences, voted_label = broadcast_data(voted_sentences), broadcast_data(voted_label)
        logger.visdom_text(text = 'start training BERT', win_name = 'state')
        sentences, labels = bert_trainer.train_model(sentences = voted_sentences,
                                                     labels = voted_label,
